...
namespace Proxy
{
    public abstract class BaseClient
    {
        ...
        private PropertyInfo[] GetPropertyInfos(Type type)
        {
            ...
            var result = type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                    .Where(p => p.CanRead && p.GetMethod.IsPublic &&
                                p.CanWrite && p.SetMethod.IsPublic).ToArray();
        ...
        protected async Task PostNoResultAsync(string relativeUri, object value)
        {
        ...
        protected async Task<T> PostAsync<T>(string relativeUri, object value)
        {
            var response = await HttpClient.PostAsJsonAsync(relativeUri, value).ConfigureAwait(false);
        ...
        protected async Task PutNoResultAsync(string relativeUri, object value)
        {
        ...
        protected async Task<T> PutAsync<T>(string relativeUri, object value)
        {
            var response = await HttpClient.PutAsJsonAsync(relativeUri, value).ConfigureAwait(false);
